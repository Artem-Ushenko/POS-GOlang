	...
	view.receipt = widget.NewList(
		...
		func(id widget.ListItemID, item fyne.CanvasObject) {
			...
			add.OnTapped = func() {
				view.withUI(func() {
					if line.Stock == 0 {
						view.setStatus("Out of stock: " + line.Name)
						return
					}
					if line.Qty >= line.Stock {
						view.setStatus("Max stock reached: " + line.Name)
						return
					}
					line.Qty++
					view.refreshReceiptUI()
					view.setStatus("Added " + line.Name)
				})
			}
...
func (c *CheckoutView) addOrIncrement(line *CartLine) {
	if existing, ok := c.cartByCode[line.Barcode]; ok {
		if existing.Stock == 0 {
			c.setStatus("Out of stock: " + existing.Name)
			return
		}
		if existing.Qty >= existing.Stock {
			c.setStatus("Max stock reached: " + existing.Name)
			return
		}
		existing.Qty++
		c.setStatus("Added " + existing.Name)
		c.refreshReceiptUI()
		return
	}
	if line.Stock == 0 {
